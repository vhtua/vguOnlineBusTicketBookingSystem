    ...
    if ($captcha_res['success']) {
        if (isset($_POST['oldpassword']) && isset($_POST['newpassword']) && isset($_POST['newpasswordagain'])) {
            ...
            if (empty($_POST['oldpassword']) || !password_verify($_POST['oldpassword'], $_SESSION['user_password'])) {
                ...
                header("Location: ../view/userdashboard/driverIndex.php?menu=account&action=change_password&error=$error");
                ...
                // echo 'Good';
                driver_change_password($_SESSION['user_id'], $_POST['newpassword'], $conn);
            }
        }
    } else {
        $error = 'Failed captcha, please try again.';
        header("Location: ../view/userdashboard/index.php?menu=account&action=change_password&error=$error");
    }
?>
